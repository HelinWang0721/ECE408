...
__global__ void matrixMultiplyShared(float *A, float *B, float *C,
  ...
  for (int m = 0; m < (numAColumns - 1)/TileWidth + 1; ++m) {
    if (m * TileWidth + tx < numAColumns && Row < numARows)
      subTileM[ty][tx] = A[Row * numAColumns + m * TileWidth + tx];
    else
      subTileM[ty][tx] = 0.0;

    if (m * TileWidth + ty < numBRows && Col < numBColumns)
      subTileN[ty][tx] = B[(m * TileWidth + ty) * numBColumns + Col];
    else
      subTileN[ty][tx] = 0.0;
